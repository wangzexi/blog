...
AI 实习生“小五”实战真实产品需求：业务页面卖点信息优化（链接已省略）
...
1. 发布系统 打包、锁包链接这类发布动作，对应业务仓库里的 workflow（链接已省略）。
2. iOS 模拟器操作，对应 ios-simulator-control（链接已省略）。
3. 获取业务场景链接，对应 room-link-query（链接已省略）。
