...
这里我特意把 `Agent Runtime` 高亮出来，因为它对应的就是第一部分讲的那个 Agent 核心循环。外围再由 `Bot 服务` 负责接入企业 IM、消息调度和回传，由 `技能系统` 负责技能沉淀。
...
2. 企业 IM 把消息回调给 `Bot 服务`。
3. `Bot 服务` 按聊天上下文派发任务。
4. `Bot 服务` 把任务交给 `Agent Runtime` 执行。
5. `Agent Runtime` 再去调用浏览器、iOS 模拟器、本地开发环境等工具。
6. 结果回传给 `Bot 服务`，再发回企业 IM。

如果压缩成一句话，就是：

> 企业 IM 负责协作场景，Bot 服务负责接入、会话派发和回传，Agent Runtime 负责推理和执行，工具环境负责把任务真正做完。
...
第五，它可以开启 `serve` 模式，SDK 也比较成熟，方便我们在外围再接 `Bot 服务`、`技能系统` 这些自定义能力。
