...
# AI 实习生：把 Agent 放进上下文已经存在的地方

## 开场

大家好，今天我想介绍的不是一个聊天机器人，而是一个在企业 IM 里工作的数字员工，名字叫 `AI 实习生`。

先从一句话定义开始，再解释它背后的架构和原理。

如果先用一句话定义它，那就是：

`AI 实习生` 是一个接入了企业 IM、拥有独立员工身份、可以调用真实工具和环境来完成任务的通用 Agent。

我今天主要讲 4 个问题：

1. 通用 Agent 到底是怎么工作的。
2. 为什么 `AI 实习生` 不是普通聊天机器人。
...
理解了通用 Agent 的工作方式之后，再回到 `AI 实习生`，事情就会清楚很多。
...
这里我特意把 `Agent Runtime` 高亮出来，因为它对应的就是第一部分讲的那个 Agent 核心循环。外围再由 `agent-gateway` 负责接入企业 IM、消息调度和回传，由 `skill-registry` 负责技能沉淀。

`AI 实习生` 的具象能力，我觉得可以先看 3 个：
...
接下来我讲一下 `AI 实习生` 的工程主干。

整个链路并不复杂，大致是这样：

1. 用户在企业 IM 群聊或者私聊里发消息。
2. 企业 IM 把消息回调给 `agent-gateway`。
3. `agent-gateway` 按聊天上下文派发任务。
4. `agent-gateway` 把任务交给 `Agent Runtime` 执行。
5. `Agent Runtime` 再去调用浏览器、iOS 模拟器、本地开发环境等工具。
6. 结果回传给 `agent-gateway`，再发回企业 IM。

如果压缩成一句话，就是：

> 企业 IM 负责协作场景，agent-gateway 负责接入、会话派发和回传，Agent Runtime 负责推理和执行，工具环境负责把任务真正做完。
...
`agent-gateway` 不只是转发一段控制台字符串，它还支持：
...
`skill-registry` 里已经有很多很典型的技能：

1. `ios-simulator-control`
2. `live-test-suite`
3. `monorepo-repo-router`
4. `bus-release`
5. `agent-gateway-self-ops`
...
`AI 实习生` 使用的是独立员工身份 `agent_staff`，而不是借用某个真人账号去执行任务。
...
## 第八部分：我们为什么选择 Agent Runtime

在这套系统里，`Agent Runtime` 不是被当成一个单纯的 IDE 工具，而是被当成一个 Agent 引擎来使用。

我们最后选择 `Agent Runtime`，原因其实很直接。
...
第五，它可以开启 `serve` 模式，SDK 也比较成熟，方便我们在外围再接 `agent-gateway`、`skill-registry` 这些自定义能力。

如果再往上说一层，我们并不是认为 `Agent Runtime` 和其他 Agent 产品在原理上有本质差异。
...
1. IDE / 终端型，比如 `Agent Runtime`、`Cloud Code`、`Cursor`。
...
## 第十部分：把这个话题再落回 AI 实习生

讲到这里，再回到 `AI 实习生`，我们的方向其实没有做得那么复杂。
